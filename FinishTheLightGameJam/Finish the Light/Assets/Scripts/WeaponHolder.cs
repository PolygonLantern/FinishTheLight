   ...
   void CheckForShooting()
   {
      if (Input.GetMouseButtonDown(0))
      {
         ...
         if (Physics.Raycast(_cameraTransform.position, transform.forward, out raycastHit, Mathf.Infinity))
         {
            ...
            if (damagable != null)
            {
               float normalisedDistance = raycastHit.distance / _equippedGun.maxRange;
               if (normalisedDistance <= 1)
               {
                  damagable.TakeDamageFromPlayer(Mathf.RoundToInt(Mathf.Lerp(_equippedGun.maxDamage, _equippedGun.minDamage, normalisedDistance)));
               }
